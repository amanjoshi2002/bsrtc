...
const BookingPolicyAdmin = () => {
  ...
  const fetchPolicies = async () => {
    ...
    try {
      const response = await axios.get(
        `${config.apiBaseUrl}/booking-policy`,
        {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setShowForm(false);
      alert('Booking Policy added successfully');
    } catch (err) {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await axios.put(
        `${config.apiBaseUrl}/booking-policy/${id}`,
        currentItem,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert('Booking Policy deleted successfully');
    } catch (err) {
  ...
  return (
    <Container maxWidth="lg">
      <Typography variant="h4" sx={{ mb: 4 }}>Booking Policy Management</Typography>
...
export default BookingPolicyAdmin; 